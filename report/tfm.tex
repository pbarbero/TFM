...
$$ E=\sum{i=1}^{n} \sum_{x\in S_i} d(x, m_i) $$
...
%La definici\'on de cl\'uster dada por \textbf{DBSCAN} es ligeramente distinta a la que hemos proporcionado antes, ya que se basa en la noci\'on de puntos \textit{densamente alcanzables}. B\'asicamente, se considera que $q$ es densamente alcanzable \textbf{directamente} desde $p$ si la distancia entre ellos no es m\'as de un $\varepsilon$ fijado y adem\'as $p$ tiene una cantidad m\'inima de puntos alrededor suficiente para formar un cl\'uster.  \\
%
%Se dice que $q$ es densamente alcanzable desde $p$ (no directamente) si existe una secuencia de puntos $p_1, \ldots, p_n$ con $p_1 = p$ y $p_n = q$ donde cada $p_{i+1}$ es densamente alcanzable desde $p_i$. \\
%
%Esta relaci\'on no es sim\'etrica. Dos puntos en el borde del mismo cl\'uster $S$ pueden no ser densamente alcanzables entre ellos 
...
\begin{itemize}
	...
	\item Un punto $q$ es considerado \textit{alcanzable} de $p$ si existe un camino $p_1, \ldots, p_n$ tal que $p_1=p$ y $p_n=q$, donde cada $p_{i+1}$ es directamente alcanzable desde $p_i$ (todos los puntos del camino son puntos n\'ucleo, excepto quizás $q$).
